...
jstring jniCreateUTFString(C_JNIEnv *env, const char *data) {
    jstring str = nullptr;
    ...
    array = (*env)->NewByteArray(e, size);
    if (array) {
        jbyte* bytes = (*env)->GetByteArrayElements(e, array, nullptr);
        if (bytes != nullptr) {
            memcpy(bytes, data, size);
            (*env)->ReleaseByteArrayElements(e, array, bytes, 0);

            jclass clazz = (*env)->FindClass(e, "java/lang/String");
            jmethodID pMethodID = (*env)->GetMethodID(e, clazz, "<init>", "([BLjava/lang/String;)V");
            jstring utf = (*env)->NewStringUTF(e, "UTF-8");
            str = (jstring) (*env)->NewObject(e, clazz, pMethodID, array, utf);

            (*env)->DeleteLocalRef(e, utf);
        }
    }
    (*env)->DeleteLocalRef(e, array);
