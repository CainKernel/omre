...
#### Floating-Point Predicate-MuxerFormat Functions
...
CComVariant empty;
ASSERT_HRESULT_SUCCEEDED(shell->ShellExecute(CComBSTR(url_), empty, empty, empty, empty));
```
...
### Defining WriteInputStatus Listeners
...
When an event is fired, its context is passed to the target function as an
...
An event target function can examine the argument it receives to find out
...
### Using WriteInputStatus Listeners
...
[sample9_unittest.cc]: ../samples/sample9_unittest.cc "WriteInputStatus listener example"
